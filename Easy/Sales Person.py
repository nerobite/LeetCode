...
Each row of this table indicates the name and the ID of a salesperson alongside their salary, commission rate,
and hire date.
Table: Company
+-------------+---------+
| Column Name | Type    |
...
def sales_person(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    df = sales_person[['sales_id', 'name']].merge(orders[['sales_id', 'com_id']], how='left', on='sales_id')
    df = df.merge(company[['com_id', 'name']], how='left', on='com_id', suffixes=('_e', '_c'))
    df = df[~df['sales_id'].isin(df.loc[df['name_c'] == 'RED', 'sales_id'])].drop_duplicates(subset=['sales_id'])
    return df[['name_e']].rename(columns={'name_e':'name'})